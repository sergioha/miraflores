-- View: servicios_sinfechaejecucion

-- DROP VIEW servicios_sinfechaejecucion;

CREATE OR REPLACE VIEW servicios_sinfechaejecucion AS 
 SELECT detalle.id, cliente.user_id, orden.id AS orden_id, orden.cantidad, orden.talla, tipo.id AS tipo_servicio_id, tipo.nombre AS nombre_tiposervicio, servicio.id AS servicio_id, servicio.nombre AS nombre_servicio, detalle.fecha_ejecucion, detalle.terminado
  ...
  WHERE servicio.tipo_servicio_id = tipo.id AND detalle.servicio_id = servicio.id AND detalle.orden_id = orden.id AND cliente.user_id = orden.cliente_id AND detalle.fecha_ejecucion IS NULL;

ALTER TABLE servicios_sinfechaejecucion OWNER TO django_login;
